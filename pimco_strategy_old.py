...
#
#total_vw_nre=pd.DataFrame(pd.Series((oil_vw_nre*(54.68/58.89)).values+(gold_vw_nre*(4.21/58.89)).values))
#
#total_vw_nre_buff=total_vw_nre[121:]
#
#vw_Ci=pd.DataFrame((ret_petCI*0.5).values+(ret_goldCI*0.5).values)
#vw_Ci_buff=vw_Ci[121:]
#
#VW_TOTAL_result=pd.DataFrame(index=np.arange(0, numberOfwindow), columns=('x1', 'x2','c') )
#text_file = open("vw_total.txt", "w")
#rsquare_sum=0
#for i in range(numberOfwindow):
#    x = []
#    x.append(vw_Ci[1+i:121+i])
#    x.append(Spx_Index[0 + i:numberOfmonthIn10Y+i])
#    result = reg_m(total_vw_nre[1 + i:121+i], x)
#    rsquare_sum = rsquare_sum+result.rsquared
#    text_file.write(result.summary().as_text())
...
#text_file.close()
#    
#vw_Broad_CI_beta =pd.DataFrame(VW_TOTAL_result['x1'])
#vw_ME_beta = pd.DataFrame(VW_TOTAL_result['x2'])
#vw_Cash_beta = pd.DataFrame(1-vw_Broad_CI_beta.values-vw_ME_beta.values)
#total_vw_nre = pd.DataFrame(total_vw_nre).values
#VW_TOTAL_result=pd.DataFrame(vw_Ci_buff.values*vw_Broad_CI_beta.values+Spx_Index_buff.values*vw_ME_beta.values+rf_buff*(1-vw_Broad_CI_beta.values-vw_ME_beta.values))
#excess_ret1=pd.DataFrame(-total_vw_nre_buff.values+VW_TOTAL_result.values)
#
#sharpe_PF_2 = (VW_TOTAL_result-rf_buff.values).mean()*12/(VW_TOTAL_result.std()*np.sqrt(12))
#sharpe_nre_2 = (total_vw_nre_buff.values-rf_buff.values).mean()*12/(total_vw_nre_buff.values.std()*np.sqrt(12))
#
#df1 = pd.DataFrame()
#df1['a'] = VW_TOTAL_result[0].values.astype(float)
#df1['b'] = total_vw_nre_buff[0].reset_index(drop=True)
#Model_corr1 = df1['a'].corr(df1['b'])
#
#print('####### 동일비중 NRE ~  동일비중 CI 회귀  #######')
#print("Rsquare Mean: %.3f" %(rsquare_sum/numberOfwindow))
#print('Model correlation = %.2f' %(Model_corr1))
#print('sh_PF_VW_TOTAL= %.2f' %(sharpe_PF_2))
#print('sh_NRE_VW_TOTAL= %.2f' %(sharpe_nre_2))
#print('Annualized return PF = %.2f%%' %(((1+VW_TOTAL_result.mean())**12 - 1)*100 ))
#print('Volatility PF = %.2f%%' %(VW_TOTAL_result.std()*100))
#print('Annualized return NRE = %.2f%%' %(((1+total_vw_nre_buff.mean())**12-1)*100))
#print('Volatility NRE = %.2f%%' %(total_vw_nre_buff.std()*100))
#print('-----------------------------------------')
#
#fig = plt.figure(4)
#plt.rcParams["figure.figsize"] = (10,6)
...
#plt.rcParams['axes.grid'] = True
#
#plt.plot(date_index.index,vw_ME_beta,'r-',label='Broad commodities composite beta')
#plt.plot(date_index.index,vw_Broad_CI_beta,'b-',label = 'S&P beta')
#plt.plot(date_index.index,vw_Cash_beta,'g-',label = 'Cash beta')
#plt.legend(loc=2, prop={'size': 7})
#plt.show()
#
#fig = plt.figure(5)
...
#
#
#VW_TOTAL_result_cum = (VW_TOTAL_result+1).cumprod()-1
#total_vw_nre_buff_cum = (total_vw_nre_buff+1).cumprod().reset_index(drop=True)-1
#excess_ret_cum1 = (excess_ret1+1).cumprod()-1
#
#plt.plot(date_index.index,total_vw_nre_buff_cum,'r-',label='NRE')
#plt.plot(date_index.index,VW_TOTAL_result_cum,'b-',label = 'Replicating portfolio')
#plt.plot(date_index.index,excess_ret_cum1,'g-',label = 'Excess return')
#plt.legend(loc=2, prop={'size': 7})
#plt.show()
#
#fig = plt.figure(6)
...
#plt.rcParams['axes.grid'] = True
#
#plt.plot(date_index.index,total_vw_nre_buff,'r-',label='NRE')
#plt.plot(date_index.index,VW_TOTAL_result,'b-',label = 'Replicating portfolio')
#plt.legend(loc=2, prop={'size': 7})
#plt.show()
#%%
#################################################
...
#plt.rcParams['axes.grid'] = True
#
#plt.plot(date_index_cum.index,(ret_goldCI['SPXGS_GTRM']+1).cumprod()-1,'r-',label='S&P GSCI GOLD')
#plt.plot(date_index_cum.index,(pd.DataFrame(gold_vw_nre)[0]+1).cumprod()-1,'b-',label = 'GOLD NRE')
#plt.legend(loc=2, prop={'size': 10  })
#plt.title('S&P GSCI GOLD & GOLD NRE Cumulative Return')
#plt.ylabel('return')
#plt.xlabel('time(Y)')
#plt.show()
#
...
#plt.rcParams['axes.grid'] = True
#
#VW_OIL_result_cum = (VW_OIL_result+1).cumprod()-1
#oil_vw_nre_buff_cum = (oil_vw_nre_buff+1).cumprod().reset_index(drop=True)-1
##excess_ret_cum2 = (excess_ret2+1).cumprod()-1
#
#plt.plot(date_index.index,oil_vw_nre_buff_cum,'r-',label='NRE')
#plt.plot(date_index.index,VW_OIL_result_cum,'b-',label = 'Replicating portfolio')
##plt.plot(date_index.index,excess_ret_cum2,'g-',label = 'Excess return')
#plt.legend(loc=2, prop={'size': 7})
#plt.show()
#
...
#plt.rcParams['axes.grid'] = True
#
#plt.plot(date_index.index,oil_vw_nre_buff,'r-',label='NRE')
#plt.plot(date_index.index,VW_OIL_result,'b-',label = 'Replicating portfolio')
#plt.legend(loc=2, prop={'size': 7})
#plt.show()
